...
class Splitter:
    """
    Creates an instance of the data splitter for cross validation
...
def cross_validation(splitting_strategy: str = 'StratifiedKFold',
    ...
    for train_idx, test_idx in splitter.split(x_train):
        tmp_train = split_maker(x_train.iloc[train_idx], y_train[train_idx])
        tmp_test = split_maker(x_train.iloc[test_idx], y_train[test_idx])
